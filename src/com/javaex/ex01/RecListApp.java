...
//		r04.setWidth(9);		
		...
		Iterator<Rectangle> rec = rList.iterator();
		while(rec.hasNext()) {	//	뒤에 더 있냐고 물어보기
			Rectangle rect = rec.next();
			sum += rect.area();
		}
	...
	public static void listAllDraw(List<Rectangle> list) {
		//	리스트 순회 : for 문을 이용한 인덱스접근은 좋은방법은 아니다
		//			>	Iterator를 이용한 접근을 추천한다
		//내용작성
		Iterator<Rectangle> rec = list.iterator();
		
		while(rec.hasNext()) {	//요소 유무 확인
			Rectangle itm = rec.next();
			itm.draw();
		}
	}
